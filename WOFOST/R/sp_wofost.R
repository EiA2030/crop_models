sp.wofost <- function(cultivar, soil.type, sdate, edate, parameter){
  ...
  for (x in dplyr::distinct(intermediate,X,Y)[,1]) { # Loop through all the individual points and simulated dates
    x <- dplyr::distinct(intermediate,X,Y)[n,1]
    y <- dplyr::distinct(intermediate,X,Y)[n,2]
    new <- subset(intermediate , X == x | Y == y) # Search in each of the points
    val <- as.integer(new$sowing_date[which.max(new$WSO)]) # Which is the simulated date with the maximum parameter value? --> Select that planting date
  ...
  grd <- st_join(st_as_sf(st_make_grid(st_as_sfc(st_bbox(st_as_sf(output,
                 ...
                 st_as_sf(output,
                          ...
                          crs = 4326))
  # Rasterize the grid
  r <- terra::rasterize(terra::vect(as(grd, "Spatial")),
